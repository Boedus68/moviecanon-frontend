...
// Query per recuperare tutti gli attori con almeno 2 film, ordinati per numero di film
async function getActors() {
  ...
  } | order(movieCount desc) [movieCount >= 2]`
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex flex-col gap-4">
            {actors.map((actor) => (
              <Link key={actor._id} href={`/actors/${actor.slug}`} className="bg-gray-800 rounded-lg p-4 flex items-center gap-4 hover:bg-gray-700 transition-colors duration-200">
                {actor.photo ? (
                  <img
                    src={urlFor(actor.photo).width(80).height(80).url()}
                ...
                <div className="flex-grow">
                  <h2 className="text-xl md:text-2xl font-bold text-yellow-400">{actor.name}</h2>
                </div>
                <div className="text-right flex-shrink-0">
                  <p className="text-2xl md:text-3xl font-bold">{actor.movieCount}</p>
                  <p className="text-sm text-gray-400">{actor.movieCount > 1 ? 'film' : 'film'}</p>
                </div>
              </Link>
            ))}
          </div>
