...
async function getDirectorData(slug) {
  const query = `*[_type == "director" && slug.current == $slug][0]{name, photo, biography, "movies": *[_type == "movie" && references(^._id)] | order(releaseYear desc) {_id, title, "slug": slug.current, poster, releaseYear}}`
...
export default async function DirectorPage({ params }) {
  ...
  if (!directorData) return <div>Regista non trovato!</div>
      ...
      <div className="container mx-auto px-4 py-8">
        <header className="mb-12">
           <div className="flex flex-col md:flex-row items-center gap-8">
            {directorData.photo && <Image src={urlFor(directorData.photo).url()} alt={`Foto di ${directorData.name}`} width={200} height={200} className="w-40 h-40 rounded-full object-cover shadow-lg"/>}
            <div className="text-center md:text-left">
              <h1 className="text-5xl font-bold text-yellow-400">{directorData.name}</h1>
              {directorData.biography && <div className="prose prose-invert max-w-2xl mt-4 text-gray-300"><PortableText value={directorData.biography} /></div>}
            </div>
          </div>
        </header>
